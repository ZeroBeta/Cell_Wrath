...
local function CreateLayoutPreview()
    ...
    layoutPreviewAnchor = CreateFrame("Frame", "CellLayoutPreviewAnchorFrame", layoutPreview, nil)
...
local function UpdateLayoutPreview()
    ...
    if selectedLayoutTable["main"]["combineGroups"] and previewMode ~= 1 then
        ...
        for i, group in ipairs(shownGroups) do
            ...
            local unitsPerColumn = min(selectedLayoutTable["main"]["unitsPerColumn"], 5)

            if selectedLayoutTable["main"]["orientation"] == "vertical" then
                header:SetSize(P.Scale(selectedLayoutTable["main"]["size"][1]), P.Scale(selectedLayoutTable["main"]["size"][2]) * unitsPerColumn + abs(unitSpacing) * (unitsPerColumn - 1))
                ...
                header:SetSize(P.Scale(selectedLayoutTable["main"]["size"][1]) * unitsPerColumn + abs(unitSpacing) * (unitsPerColumn - 1), P.Scale(selectedLayoutTable["main"]["size"][2]))
                for j = 1, unitsPerColumn do
                    P.Size(header[j], selectedLayoutTable["main"]["size"][1], selectedLayoutTable["main"]["size"][2])
                    header[j]:ClearAllPoints()
...
local function CreateNPCPreview()
    ...
    npcPreviewAnchor = CreateFrame("Frame", "CellNPCPreviewAnchorFrame", npcPreview, nil)
...
local function CreatePetPreview()
    ...
    petPreviewAnchor = CreateFrame("Frame", "CellPetPreviewAnchorFrame", petPreview, nil)
...
local function CreateSpotlightPreview()
    ...
    spotlightPreviewAnchor = CreateFrame("Frame", "CellSpotlightPreviewAnchorFrame", spotlightPreview, nil)
...
local function CreateLayoutPane()
    ...
    newBtn:SetTexture("Interface\\AddOns\\Cell_Wrath\\Media\\Icons\\create", {16, 16}, {"CENTER", 0, 0})
    ...
    renameBtn:SetTexture("Interface\\AddOns\\Cell_Wrath\\Media\\Icons\\rename", {16, 16}, {"CENTER", 0, 0})
    ...
    deleteBtn:SetTexture("Interface\\AddOns\\Cell_Wrath\\Media\\Icons\\trash", {16, 16}, {"CENTER", 0, 0})
    ...
    importBtn:SetTexture("Interface\\AddOns\\Cell_Wrath\\Media\\Icons\\import", {16, 16}, {"CENTER", 0, 0})
    ...
    exportBtn:SetTexture("Interface\\AddOns\\Cell_Wrath\\Media\\Icons\\export", {16, 16}, {"CENTER", 0, 0})
    ...
    shareBtn:SetTexture("Interface\\AddOns\\Cell_Wrath\\Media\\Icons\\share", {16, 16}, {"CENTER", 0, 0})
...
local function CreateLayoutSetupPane()
    ...
    hideSelfCB:SetPoint("TOPLEFT", 5, -27)
...
LoadLayoutDB = function(layout, dontShowPreview)
    ...
    -- force separated groups now that the combine option is removed
    if selectedLayoutTable["main"]["combineGroups"] then
        selectedLayoutTable["main"]["combineGroups"] = false
        if selectedLayout == Cell.vars.currentLayout then
...
LoadLayoutAutoSwitchDB = function()
    ...
    elseif Cell.isCata or Cell.isWrath then
        ...
        if Cell.vars.activeTalentGroup == 1 then
            currentProfileBox.text:SetText("|TInterface\\AddOns\\Cell_Wrath\\Media\\Icons\\1:13|t "..L["Primary Talents"])
        else
            currentProfileBox.text:SetText("|TInterface\\AddOns\\Cell_Wrath\\Media\\Icons\\2:13|t "..L["Secondary Talents"])
        end
        raid10Dropdown:SetSelectedValue(Cell.vars.layoutAutoSwitch["raid10"])
        raid25Dropdown:SetSelectedValue(Cell.vars.layoutAutoSwitch["raid25"])
        bg15Dropdown:SetSelectedValue(Cell.vars.layoutAutoSwitch["battleground15"])
